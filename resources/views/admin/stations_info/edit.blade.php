...
<div class="container mx-auto p-6">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-800">Edit Station</h2>
    </div>
    <form action="{{route('admin.station_info.update',$station_info->id)}}" method="POST" class="bg-white border border-gray-200 rounded-lg shadow-md p-6 space-y-4 w-full max-w-2xl">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" id="name"
                   value="{{ old('name', $station_info->name) }}"
        ...
        <div>
            ...
            <input type="text" name="code" id="code"
                   value="{{ old('code', $station_info->code) }}"
                   class="w-full border border-gray-300 rounded-md px-4 py-2 uppercase focus:outline-none focus:ring-2 focus:ring-indigo-500">
            @error('code')
                <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <!-- Total Platforms -->
        <div>
            <label for="total_no_platforms" class="block text-sm font-medium text-gray-700 mb-1">Total Platforms</label>
            <input type="number" name="total_no_platforms" id="total_no_platforms" min="1"
                   value="{{ old('total_no_platforms', $station_info->total_no_platforms) }}"
                   class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
