...
@section('title', 'Add Train Info')

@section('content')
<div class="container mx-auto p-6 train">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-800">Add New Train</h2>
    </div>
    @if ($errors->any())
...
@endif
    <form action="{{route('admin.train_info.store')}}" method="POST" class="bg-white border border-gray-200 rounded-lg shadow-md p-6 space-y-4 w-full max-w-2xl" id="routeForm">
        @csrf
        <div>
    ...
    <input type="hidden" name="route-id" id="route-id" value="{{ old('route-id') }}">
   ...
   <div id="train-classes-container">
    <div class="train-class-row flex items-center space-x-4 mb-2">
        ...
        <button 
            type="button" 
            class="text-red-600 hover:text-red-800 text-sm font-medium remove-btn"
        >
            Remove
        </button>
    </div>
...
let train_parent=document.querySelector('#train-classes-container');
train_parent.addEventListener('click', function(event) {
    if(event.target && event.target.classList.contains('remove-btn')){
        event.preventDefault();
        event.target.closest('.train-class-row').remove();
    }
});
</script>
@endsection
